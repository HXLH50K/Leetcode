...
class Solution:
    def maxCandies(
        ...
        own_boxes = initialBoxes
        cant_open_own_boxes = []
        own_keys = set()
        candy_cnt = 0
        flag = False

        while own_boxes or cant_open_own_boxes:
            if not own_boxes:
                if flag:
                    # 手上有一直无法处理的盒子
                    break
                own_boxes = [x for x in cant_open_own_boxes]
                flag = True

            box = own_boxes.pop(0)
            if status[box] != 1 and box not in own_keys:
                cant_open_own_boxes.append(box)
                continue

            flag = False
            candy_cnt += candies[box]
            own_keys.discard(box)
            status[box] = 0
            candies[box] = 0
